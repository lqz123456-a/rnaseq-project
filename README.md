...
- 富集分析使用同时满足 `padj < 0.05` 和 `|log2FC| > 1` 的 **844 个基因**；本次 GO BP 结果包含 **611 个显著条目**，KEGG 结果包含 **29 条显著通路**。在线数据库更新或重新运行可能产生变化。
...
`padj` 是经过 Benjamini-Hochberg 多重检验校正后的显著性指标，不包含效应量阈值。`log2FC` 表示处理组相对于对照组的表达变化幅度，正值为上调、负值为下调。因此，3,387 个显著基因与 844 个同时满足 `|log2FC| > 1` 的基因属于两个不同的筛选层级，应分别说明。当前 GO/KEGG 富集分析采用后者，以同时控制统计显著性和最小效应量。
